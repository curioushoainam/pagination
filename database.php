...
function count_all_member($username = '')
{
    ...
    if ($username){
        $query = mysqli_query($conn, 'select count(*) as total from member where username like \'%'.mysqli_real_escape_string($conn, $username).'%\'');
    }
    else{
...
function get_all_member($username = '', $limit = 10, $start = 0)
{
    ...
    if ($username){
        $sql = 'select * from member  where username like \'%'.mysqli_real_escape_string($conn, $username).'%\' limit '.(int)$start . ','.(int)$limit;
    }
    else{
